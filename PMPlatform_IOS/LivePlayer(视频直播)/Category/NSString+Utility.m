...
+ (NSString *)getSecretSignWithDeviceId:(NSString *)deviceId timestamp:(long long)timestamp {
    ...
    // 这里需要用短的appsign
    NSString *originalSign = kSecretSign;
    NSString *appSignature = [[NSString alloc] initWithBytes:[originalSign UTF8String] length:originalSign.length/2 encoding:NSUTF8StringEncoding];
    NSInteger platform = 3;
    NSInteger version = 1;

    NSString *string = [NSString stringWithFormat:@"%@%@%ld%ld%lld", appSignature, deviceId, (long)platform, (long)version, timestamp];
...
+ (NSString *)appVersion {
    return [NSString stringWithFormat:@"%@.%@", [UIApplication sharedApplication].appVersion, [UIApplication sharedApplication].appBuildVersion];
}

+ (NSString *)systemVersion {
    return [NSString stringWithFormat:@"iOS_%.1f", [UIDevice systemVersion]];
}

+ (NSString *)logFileName {
    NSTimeInterval interval = [[NSDate date] timeIntervalSince1970];
    NSString *dateStr = [NSString timeStringWithZoneOfMS:interval];
    NSString *userID = [TLManager sharedInstance].isLogin ? [NSString stringWithFormat:@"%ld", (long)[TLManager sharedInstance].userID] : [ZegoRoomKit deviceID];
    return [NSString stringWithFormat:@"%@_iOS_%@_%@_%@.zip", dateStr, @"roomkit", [NSString appVersion], userID];
}

+ (NSString *)timeStringWithZoneOfMS:(NSTimeInterval)millisecond {

...
- (void)setViewInterface {
    ...
    [pickerView selectRow:[selectedYear integerValue] - 2010 inComponent:0 animated:YES];
...
- (void)buttonTapped:(UIButton *)sender {
    if (sender.tag == 10) {
        ...
        NSString *month = [selectecMonth copy];
        month = [month stringByReplacingOccurrencesOfString:@"月" withString:@""];
        if(month.length < 2){
            month = [NSString stringWithFormat:@"0%@",month];
        }
        restr = [NSString stringWithFormat:@"%@.%@",selectedYear,month];

...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        vertexView = [[UILabel alloc] initWithFrame:CGRectZero];
...
- (void)setDatasource:(id<XCMultiTableViewDataSource>)datasource_ {
    if (datasource != datasource_) {
        ...
        responseLeftHeaderText = [datasource_ respondsToSelector:@selector(leftHeaderTextInTableView:)];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == leftHeaderTableView) {
        ...
        [datasource tableViewForLeft:target didSelectRowAtIndexPath:indexPath];
...
- (void)reset {
    leftHeaderWidth = [self leftHeaderWidth];
    ...
    [self leftHeaderView:vertexView];
...
- (void)leftHeaderView:(UIView *)container{
    ...
    label.text = [self leftHeaderText];
...
- (void)setUpTopHeaderScrollView {
    ...
    for (int i = 0; i < count; i++) {
        ...
        NSString *item = [[datasource arrayDataForTopHeaderInTableView:self] objectAtIndex:i];
        label.text = item;
...
- (UITableViewCell *)leftHeaderTableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    UILabel *label =  [[UILabel alloc] initWithFrame:CGRectMake(0, 0, leftHeaderWidth, cellH)];
    DataItem *item = [[leftHeaderDataArray objectAtIndex:indexPath.section] objectAtIndex:indexPath.row];
    label.text = item.keyName;
    [label setTextAlignment:NSTextAlignmentCenter];
...
- (NSUInteger)rowsInSection:(NSUInteger)section {
    return  1;// [leftHeaderDataArray objectAtIndex:section].count;
}
...
- (NSString *)leftHeaderText {
    return responseLeftHeaderText ? [datasource leftHeaderTextInTableView:self] : @"";
}

...
@objc public enum IndicatorStyle : Int {
...
class SGPageTitleViewConfigure: NSObject {
    ...
    /// 是否显示底部分割线，默认为 true
    @objc var showBottomSeparator: Bool = true
    /// SGPageTitleView 底部分割线颜色，默认为 lightGray
    var bottomSeparatorColor: UIColor = .lightGray
    
    // MARK: - 标题属性
    /// 标题文字字号大小，默认 15 号字体
    @objc var titleFont: UIFont = UIFont.systemFont(ofSize: 15)
    /// 标题文字选中字号大小，默认 15 号字体。一旦设置此属性，titleTextZoom 属性将不起作用
    @objc var titleSelectedFont: UIFont = UIFont.systemFont(ofSize: 15)
    /// 普通状态下标题文字的颜色，默认为黑色
    @objc var titleColor: UIColor = .black
    /// 选中状态下标题文字的颜色，默认为红色
    @objc var titleSelectedColor: UIColor = .red
    ...
    /// 标题额外增加的宽度，默认为 20.0f
    @objc var titleAdditionalWidth: CGFloat = 20.0
    ...
    /// 指示器高度，默认为 2.0f
    @objc var indicatorHeight: CGFloat = 2.0
    /// 指示器动画时间，默认为 0.1f，取值范围 0 ～ 0.3f
    var indicatorAnimationTime: TimeInterval = 0.1
    /// 指示器样式，默认为 Default
    @objc var indicatorStyle: IndicatorStyle = .Default
    /// 指示器圆角大小，默认为 0.0f
    @objc var indicatorCornerRadius: CGFloat = 0.0
    ...
    @objc var indicatorDynamicWidth: CGFloat = 20.0

...
class SGPageTitleButton: UIButton {
 ...
 @objc protocol SGPageTitleViewDelegate : NSObjectProtocol {
    /// index 当前选中标题对应的下标值
...
class SGPageTitleView: UIView {
    ...
    var resetIndex: Int?
    /// 拓展 init 方法
    @objc public init(frame: CGRect, delegate: SGPageTitleViewDelegate, titleNames: [NSString], configure: SGPageTitleViewConfigure) {
        super.init(frame: frame)
        self.backgroundColor = UIColor.white.withAlphaComponent(0.77)
        self.delegateTitleView = delegate
        self.titleNames = titleNames as [String]
    ...
    /// 重置指示器颜色
    @objc public func resetIndicatorColor(color: UIColor) {
...
extension SGPageTitleView {
    private func setupTitleButtons() {
        ...
        if allWidth <= selfSize.width { /// CoatTitleView 静止样式
            let btnY: CGFloat = 0
            let btnW: CGFloat = self.frame.size.width / CGFloat(titleCount)
...
extension SGPageTitleView {
    ...
    @objc public func setPageTitleView(progress: CGFloat, originalIndex: Int, targetIndex: Int) {
...
extension SGPageTitleView {
     ...
     */
    @objc public func setAttributedTitle(attributedTitle:NSMutableAttributedString,selectedAttributedTitle:NSMutableAttributedString,index:NSInteger){
        let btn = btnMArr[index]
        btn.setAttributedTitle(attributedTitle, for: .normal)
        btn.setAttributedTitle(selectedAttributedTitle, for: .selected)
    }
    /// 根据标题下标值添加 badge
    @objc public func addBadge(index: Int) {
        let btn = btnMArr[index]

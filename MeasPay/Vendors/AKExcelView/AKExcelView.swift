    ...
    private func configCells(collectionView:UICollectionView ,cell:AKExcelCollectionViewCell ,indexPath: IndexPath) {
        ...
        if collectionView == headFreezeCollectionView {
            ...
            if indexPath.section % 2 == 0{
                ...
                cell.backgroundColor = UIColor(red: 239/255.0, green: 238/255.0, blue: 239/255.0, alpha: 1.0)
            }
     ...
     func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
         ...
         if (contentData?.count == 0 || indexPath.item >= self.dataManager.slideItemSize.count ) {
            return CGSize.zero
        } else {
            if (collectionView == headFreezeCollectionView ||
                collectionView == contentFreezeCollectionView) {
//                Log(indexPath.item)
                return NSCoder.cgSize(for: self.dataManager.freezeItemSize[indexPath.item]);
            } else {
//                Log(indexPath.item)
                
                return NSCoder.cgSize(for: self.dataManager.slideItemSize[indexPath.item]);
            }
        }
    }
...
extension AKExcelView {
    ...
    fileprivate func scrollEndAnimation(scrollView: UIScrollView) {
        ...
        let deltaOffSets = dataManager.slideItemOffSetX.compactMap({ (offset) -> CGFloat in
        ...
        let index = deltaOffSets.firstIndex(of: min)

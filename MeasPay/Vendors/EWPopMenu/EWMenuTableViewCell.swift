...
class EWMenuTableViewCell: UITableViewCell {
    ...
    public lazy var conLabel: UILabel = {
        let label = UILabel(frame: CGRect(x: 0, y: 0, width: itemWidth - 57, height: itemHeight))
...
//        self.addSubview(iconImg)
    ...
    public func setContentBy(titArray: [String], imgArray: [String], row: Int){
        if imgArray.count == 0{
            ...
            self.conLabel.frame = CGRect(x: 6, y: 0, width: itemWidth - kLineXY * 2, height: itemHeight)
        }else{

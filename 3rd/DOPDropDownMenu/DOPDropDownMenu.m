...
-(void)setShow:(BOOL)show{
    ...
    if (show) {
        if (self.showBlock) {
            self.showBlock(self.currentIndexPath);
        }
    }
    else{
...
- (instancetype)initWithOrigin:(CGPoint)origin width:(CGFloat)width andHeight:(CGFloat)height {
    ...
    if (self) {
        ...
        //lefttableView init
        _leftTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, self.frame.origin.y + self.frame.size.height, dropDownViewSize.width/2, 0) style:UITableViewStylePlain];
        ...
        //righttableView init
        _rightTableView = [[UITableView alloc] initWithFrame:CGRectMake(0 + _dropDownViewWidth/2, self.frame.origin.y + self.frame.size.height, dropDownViewSize.width/2, 0) style:UITableViewStylePlain];
        ...
        _buttomImageView = [[UIImageView alloc]initWithFrame:CGRectMake(0, self.frame.origin.y + self.frame.size.height, dropDownViewSize.width, kButtomImageViewHeight)];
        ...
        _backGroundView = [[UIView alloc] initWithFrame:CGRectMake(0, origin.y + height, dropDownViewSize.width, dropDownViewSize.height)];
...
- (CATextLayer *)createTextLayerWithNSString:(NSString *)string withColor:(UIColor *)color andPosition:(CGPoint)point {
    NSString *fixString = [string stringByReplacingOccurrencesOfString:@"全部" withString:@""];
    CGSize size = [self calculateTitleSizeWithString:fixString];
    
    CATextLayer *layer = [CATextLayer new];
    CGFloat sizeWidth = (size.width < (self.frame.size.width / _numOfMenu) - 25) ? size.width : self.frame.size.width / _numOfMenu - 25;
    layer.bounds = CGRectMake(0, 0, sizeWidth, size.height);
    layer.string = fixString;
...
- (void)animateBackGroundView:(UIView *)view show:(BOOL)show complete:(void(^)(void))complete {
    if (show) {
        //调整高度，避免多余背景
        CGFloat hight = self.superview.frame.size.height - self.frame.size.height;
...
- (void)animateIdicator:(id)indicator background:(UIView *)background tableView:(UITableView *)tableView title:(CATextLayer *)title forward:(BOOL)forward complecte:(void(^)(void))complete{
    if (self.indicatorAlignType == DOPIndicatorAlignTypeCloseToTitle) {
        [self layoutIndicator:indicator withTitle:title];
    }

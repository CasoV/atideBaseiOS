...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [cell setNeedsLayout];
    NSArray <NSString *>* arr = [item.dateStr componentsSeparatedByString:@"-"];
    if ((arr.count == 3) && self.currentDate){
        NSString *str = [NSString stringWithFormat:@"%04ld-%02ld-%02ld", (long)arr[0].integerValue, (long)arr[1].integerValue, (long)arr[2].integerValue];
        NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
        formatter.locale = [NSLocale currentLocale];
        formatter.timeZone = [NSTimeZone localTimeZone];
        formatter.dateFormat = @"yyyy-MM-dd";
        if ([str isEqualToString:[formatter stringFromDate:self.currentDate]]) {
            if (_sign) {
                _sign = !_sign;
...
- (BOOL)isSelected {
    for (UICollectionViewCell *cell in _calView.visibleCells) {
        if (cell.isSelected) {
            return YES;
        }
    }

...
-(void)setUpChart{
	...
	}else if (self.chartStyle == UUChartBarStyle)
	{
        if (!_barChart) {
            ...
            _barChart = [[UUBarChart alloc] initWithFrameAndSep:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height) barLableSep:yBarLabelWidth];
            [self addSubview:_barChart];
        }
        //设置左边label的宽度
        _barChart.yBarLableWidth = yBarLabelWidth;
        _barChart.itemSepWidth = itemSepWidth;
        [_barChart setBarNum:barNum];
        if ([self.dataSource respondsToSelector:@selector(UUChartChooseRangeInLineChart:)]) {
